    ...
    <div className="w-3/5 h-full flex flex-col items-center justify-center">
      <div className="flex flex-col w-4/5 h-1/2 justify-between">
        ...
        <form className="flex flex-col justify-between h-3/5 pt-6">
          <input type="text" placeholder="Email" value={email} onChange={ (e) => setEmail(e.target.value) } className="w-96 border-b-2 border-black focus:outline-none"/>
          <input type="password" placeholder="Password" value={password} onChange={ (e) => setPassword(e.target.value) } className="w-96 border-b-2 border-black focus:outline-none"/>
          <input type="password" placeholder="Confirm Password" value={passwordConfirmation} onChange={ (e) => setPasswordConfirmation(e.target.value) } className="w-96 border-b-2 border-black focus:outline-none"/>
          <button onClick={register} className="h-14 w-96 bg-way-light-gray text-bg-way-dark-gray focus:outline-none">Register</button>
        </form>
